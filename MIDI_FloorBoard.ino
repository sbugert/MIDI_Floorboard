...
#define MIDICHANNEL 0    // MIDI-Kanal: 0-15 für Kanal 1-16
#define VELOCITY 69
const unsigned char noteValue[INPUTS] = {
  60,62,64,65,
  67,69,71,72,
  74,76,77,79,
  81,83,84,86};  // Array mit Midi-Noten
...
boolean activeSwitch[INPUTS] = {
  ...
  HIGH,HIGH,HIGH,HIGH,
  HIGH,HIGH,HIGH,HIGH,
  HIGH,HIGH,HIGH,HIGH};     // gerade gedrückte Taster
...
void setup() {
  ...
  pinMode(Z, INPUT);
  digitalWrite(Z, HIGH); // internen Pull-Up aktivieren
  
//  analogReadResolution(8);    // ADC auf 8-Bit-Auflösung setzen
...
void loop() {
  for (int i = 0; i < INPUTS; i++) {
    if(readSwitch(i) == LOW && activeSwitch[i] == HIGH) {             // Wenn Taster i gedrückt ist
      activeSwitch[i] = LOW;
